...
import { CustomerRepository } from './User';
...
class CustomerController {
  ...
  async createCustomer(req: Request, res: Response) {
    ...
    if (data?.statusCode == 201) {
      return ResponseHandler.successResponse(res, data.statusCode, data.message, data.body);
    }
    return ResponseHandler.errorResponse(res, data.statusCode, data.message, data.body);
  }

  async getCustomer(req: Request, res: Response) {
    const { email: username, password } = req.body;
    const options = getOptions(req.headers, { username, password });
    const data = await new CustomerRepository(options).getCustomer(req.body, res.locals);
